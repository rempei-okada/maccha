...
import { appendInitialData } from "../InitialData/appendInitialData";

export class Init1602430030211 implements MigrationInterface {
    name = "Init1602430030211";
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query("CREATE TABLE `post_type_entity` (`postTypeId` varchar(36) NOT NULL, `taxonomyId` varchar(128) NOT NULL, `identifier` varchar(255) NOT NULL, `isDeleted` tinyint NOT NULL, INDEX `IDX_7c0a0c1c20c138016f362240f9` (`identifier`), UNIQUE INDEX `REL_4ad0a6c725ceace2c07c9df8ef` (`taxonomyId`), PRIMARY KEY (`postTypeId`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `scheme_entity` (`schemeId` varchar(36) NOT NULL, `taxonomyId` varchar(255) NOT NULL, `type` varchar(64) NOT NULL, `name` varchar(64) NOT NULL, `displayName` varchar(64) NOT NULL, `description` varchar(512) NOT NULL, PRIMARY KEY (`schemeId`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `field_entity` (`fieldId` varchar(36) NOT NULL, `name` varchar(128) NOT NULL, `type` varchar(128) NOT NULL, `value` longtext NOT NULL, `contentId` varchar(255) NOT NULL, PRIMARY KEY (`fieldId`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `content_entity` (`contentId` varchar(36) NOT NULL, `identifier` varchar(255) NOT NULL, `description` varchar(255) NOT NULL, `taxonomyId` varchar(255) NOT NULL, `title` varchar(255) NOT NULL, `thumbnail` varchar(255) NOT NULL, `status` int NOT NULL, `metadata` text NOT NULL, `updatedAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `createdAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `publishIn` datetime NOT NULL, `createdBy` varchar(255) NOT NULL, INDEX `IDX_bd9d611a55197686ce0801062b` (`identifier`), PRIMARY KEY (`contentId`)) ENGINE=InnoDB");
        ...
        await queryRunner.query("ALTER TABLE `field_entity` ADD CONSTRAINT `FK_a1169144cd45b3aecb09037507b` FOREIGN KEY (`contentId`) REFERENCES `content_entity`(`contentId`) ON DELETE NO ACTION ON UPDATE NO ACTION");
        await appendInitialData(queryRunner);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("ALTER TABLE `field_entity` DROP FOREIGN KEY `FK_a1169144cd45b3aecb09037507b`");
